...
//	char *asciiGenotypes;
	 ...
	 */
	int ImportCompressedBinSnp(uint linenumber, uint label, uint32_t values[], const int count = 1, const int framesize = 2);
	...
	void SetLastEvaluation(float score);
...
inline
void SnpAligned::SetLastEvaluation(float score) {
	lastMdEval = score;
}

inline
float SnpAligned::GetLastMdEval() {
	return lastMdEval;
}
...
void SnpAligned::SetIndividualCount(int individualCount) {
	...
	cout<<"Attempting to Initializing Bitsets\n";
...
inline
SnpAligned::SnpAligned(int genotypeCount, uint id) : genotypeCount(genotypeCount), individualCount(0), binaryLabel(NULL), id(id), lastMdEval(-1), _instanceCount(0) {
