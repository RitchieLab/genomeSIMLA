...
int SnpAligned::ImportBpSnp(uint linenumber, uint label, const char *line) {
	...
	char_separator<char> sep(" \t,", "", boost::drop_empty_tokens);
...
int SnpAligned::ImportCompressedBinSnp(uint linenumber, uint label, uint32_t values[], const int count/* = 1*/, const int framesize/* = 2*/)
{
	 ...
	 */
	BinArrayParser<uint32_t> binParser(values, count, framesize);
	bitsets[0].set();
	//int pos=0;
	for (uint i=0; i<individualCount; i++)		{
