...
FamilyMember *FamilyNode::CreateVirtualSib(FamilyMember *member) {
	...
	if (member->IsAffected()) {
		...
		//Make sure we don't create any meaningless siblings
		if (fatherData->CountGenotypesPresent() * motherData->CountGenotypesPresent() == 0) {
			member->MarkForDeletion( true );
			if (os)
				*os<<"Trio: "<<member->GetFamilyID()<<"\t\tUnable to create virtual sibling due to lack of parental genotypes\n";
			return NULL;
		}
		if (os) {
...
uint FamilyNode::BuildDsps(uint totalIndividualCount, bool createVirtSibs) {
	map <string, FamilyMember *>::iterator itr = entries.begin();
	map <string, FamilyMember *>::iterator end = entries.end();
...
uint FamilyNode::AddGenoData(vector<GenotypeData *> &data, CaseControlStatus ccData[], CaseControlStatus pgData[], uint &ccPosition, uint &pgPosition,  bool doScrambleStatus /* = false */) {
	ExtendedSibship::iterator end = sibships.end();
	...
	while (cur != end) {
		...
		if (aIdx > 0 && uIdx != unit->affectedCount)
		{
			...
			if (doScrambleStatus) 
				random_shuffle(children, children+uIdx);
			//Basic book keeping
	...
	//Push this local status onto the stack of current statuses
	ccData[ccPosition++] = status;	
