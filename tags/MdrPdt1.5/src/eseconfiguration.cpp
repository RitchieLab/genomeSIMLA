...
EseConfiguration::EseConfiguration() : pTestCount(1000), pValueThreshold(0.30), minDThresh(0.0), pTestSeed(13), logLocus(NULL), comboEnd(1),comboStart(1), fDist(NULL), oDist(NULL),  evalSuite(NULL), inputParser(NULL), logDistribution(NULL), logReport(NULL), logPedigree(NULL), performPerfectSearch(false)
{
	families = 	NULL;
	errorMsg="";
	SetDefaultValues();
...
SnpEvalSuite *EseConfiguration::GetPdtEval(bool doRandomizeStatus, uint testNumber) {
	...
	if (pTestCount > 0 && !oDist)
		oDist = new NTest(comboEnd, pTestSeed, "M. Odds Ratio");
	SnpEvalSuite *evalSuite=new SnpEvalSuite(evalPDT, fDist, oDist);
