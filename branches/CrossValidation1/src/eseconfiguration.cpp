...
bool EseConfiguration::SetValues(const char *key, const char *value) {
	bool success=true;
	assert(strlen(key)>0);
...
//			abort();
			...
			EvalMaxDifference *md = new ESE::EvalMaxDifference(bestDifferenceThreshold, comboEnd);
			...
			//Build PTests
			OmnibusDistribution *distribution=new OmnibusDistribution(pTestCount, 0, GetDistributionLog());
			evalSuite->SetDistribution(distribution);
/*			
...
GtFileParser *EseConfiguration::GetInputFileParser() {
...
void EseConfiguration::GenerateReport(ostream& os) {
	...
	switch (analysisStyle) {
		...
		case EseConfiguration::MaxDiff:
			...
			break;
		case EseConfiguration::BalancedAccuracy:
			cout<<"Balanced Accuracy. Threshold is based on cases and controls counts available at the given model.\n";
			break;
		case EseConfiguration::PDT:
