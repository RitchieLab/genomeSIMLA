...
//std::cerr<<" -- US: "<<this->idx<<"\t"<<idx<<"\t"<<lIdx<<"\t"<<dp<<"\t"<<rs<<"\n";
...
//std::cerr<<" -- DS: "<<this->idx<<"\t"<<idx<<"\t"<<lIdx<<"\t"<<dp<<"\t"<<rs<<"\n";
...
//std::cerr<<" vv "<<" "<<idx<<" "<<pos<<" : "<<" "<<minDP<<" | "<<itr->dp<<" "<<itr->rs<<"\n";
		doContinue = itr->rs == -1 || itr->rs >= minRS;
		if (doContinue) {
			if (itr->rs > 0.0) {
//std::cerr<<" "<<this->idx<<" vv "<<" "<<this->idx + i<<" "<<pos<<" : "<<" "<<minDP<<" | "<<itr->dp<<" "<<itr->rs<<"\n";
	...
	while (itr!=end && doContinue) {
		i++;
//std::cerr<<" ^^ "<<" "<<idx<<" "<<pos<<" : "<<" "<<minDP<<" | "<<itr->dp<<" "<<itr->rs<<"\n";
		doContinue = (itr->rs == -1 || itr->rs>=minRS);
		if (doContinue) {
			if (itr->rs > 0){
//std::cerr<<" "<<this->idx<<" ^^ "<<" "<<this->idx+i<<" "<<pos<<" : "<<" "<<minDP<<" | "<<itr->dp<<" "<<itr->rs<<"\n";
			indexes[this->idx+i]= itr->rs;
				//indexes.push_back(this->idx + i + 1);
			}
		}
		itr++;
	}
...
//assert(sItr->rs >= 0.0 && sItr->dp>=0.0);
if (willfail)
	std::cerr<<" vv rs"<<rs<<"\t"<<pos<<"\t"<<sItr->dp<<"\t"<<sItr->rs<<"\n";
...
//assert(sItr->rs >= 0.0 && sItr->dp>=0.0);
if (willfail)
	std::cerr<<" ^^ rs"<<rs<<"\t"<<pos<<"\t"<<sItr->dp<<"\t"<<sItr->rs<<"\n";
		sItr++;
	}
