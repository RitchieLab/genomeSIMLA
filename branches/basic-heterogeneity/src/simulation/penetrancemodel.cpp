...
const char *PenetranceModel::BasicHetero = "BASIC_HETERO";
...
PenetranceModel *PenetranceModel::GetModel(istream& ss, PoolManager *pools, std::vector<std::string>& additionalCfg) {
	...
	else if (strcmp(modelType.c_str(), BasicHetero) == 0) {
		model = new BasicHeterogeneity();
		cout<<"Basic Heterogeneity\n";
		std::vector<std::string>::iterator itr = additionalCfg.begin();
		std::vector<std::string>::iterator end = additionalCfg.end();

		while (itr != end) {
			model->AddModel(itr++->c_str());
		}
	}
