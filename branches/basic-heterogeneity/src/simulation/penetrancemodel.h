...
public:
	...
	/**
	 * By default, we won't do anything, but when adding a model to the model makes sense...
    * @param line
    */
	virtual void AddModel(const char *line) {}
	/**
	 ...
	 */
	static PenetranceModel *GetModel(istream& details, PoolManager *pools, std::vector<std::string>& additionalCfg);
