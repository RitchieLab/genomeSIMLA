...
CPPUNIT_TEST_SUITE_REGISTRATION( MDRPDT::Test::TestPdtTStat );
...
void TestPdtTStat::SetupRepository(const char *filename, bool doCreateVSibs) {
	...
	families->Purge(); 
	pdtParser=new GtLineParserMdrPdt(2, false, families, 4 );
	pdtParser->DoCreateVirtualSibs(doCreateVSibs);
...
void TestPdtTStat::VerifyTScores(string models[], float tValues[], uint modelCount ) {
	...
	evalPDT->EvaluateVerbose( model );
	CPPUNIT_ASSERT_DOUBLES_EQUAL(tValues[0], model->GetLastMdEval(), 0.001);
