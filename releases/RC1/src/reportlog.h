...
void ReportLog::ReportOnRepository(const char *title, uint locusCount, SnpReporter *snps) {
	...
	for (uint loci=0; loci<locusCount; loci++) {
		if (locusCount>0) 
			sprintf(entryText, "%d SNP-models %d/%d models reported/found meeting or exceeding threshold", loci+1, snps->GetEntryCount(loci), snps->GetTotalEntryCount(loci));
		else  
			sprintf(entryText, "%d/%d models reported/found meeting or exceeding threshold", snps->GetEntryCount(loci), snps->GetTotalEntryCount(loci));
		
			Echo(entryText);
		uint count=snps->GetEntryCount(loci);
		if (count > 0) {
			EchoTitle(title);
			WriteHeader(loci);
			for (uint i=0; i<count; i++) {
				Echo(i, snps->GetReportEntry(loci, i).c_str());
			}
		}
		else {
