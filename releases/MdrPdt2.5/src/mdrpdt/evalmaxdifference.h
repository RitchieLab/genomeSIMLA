...
public:
	 ...
	 */
	ModelStatistics GetModelStats(SnpAligned *&snp)	{ assert(0); }
	/**
	 * @brief Used for X-Validation, we will search through the folds for best model for a given size
	 */
	ModelStatistics GetModelStats(uint modelSize, SnpAligned *&snp)	{ assert(0); }
	FoldStatistics GetFoldStatistics( uint modelSize, SnpAligned *snp)	{ assert(0); }
