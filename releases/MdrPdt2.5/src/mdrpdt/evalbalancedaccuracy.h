...
public:
	...
	float GetAverageTesting(SnpAligned *& snp) { assert(0); }

	/**
	 * @brief Used for X-Validation, we will search through the folds for the best snp considering all model sizes.
	 */
	Genetics::Evaluation::ModelStatistics GetModelStats(SnpAligned *&snp)	{ assert(0); }
	/**
	 * @brief Used for X-Validation, we will search through the folds for best model for a given size
	 */
	Genetics::Evaluation::ModelStatistics GetModelStats(uint modelSize, SnpAligned *&snp)	{ assert(0); }
	Genetics::Evaluation::FoldStatistics GetFoldStatistics( uint modelSize, SnpAligned *snp)	{ assert(0); }
