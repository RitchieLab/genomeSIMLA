...
// -------------------------------- M A I N -----------------------------------
float run_simpen(const char *filename, const char *output, LocusArray &loci, int seed, bool override) {
...
string CreateDatasimFile(string base_file, GARealGenome& genome, int num_loci, 
  ...
  string datasim_file = base_file + "." + Stringmanip::itos(modNum) + ".datasim";
  ofstream datasimFile(datasim_file.c_str(), ios::out);
...
string CreateModelFile(string base_file, int num_loci, GARealGenome& genome,
  const vector<string> & labels, int modNum, LocusArray &loci){
	string freq_file = base_file + "." + Stringmanip::itos(modNum) + ".smod";
	ofstream fFile(freq_file.c_str(), ios::out);
	...
	// add loci spaced 5 apart
  	...
  	for(int i=0; i<num_loci; i++){
		fFile<<loci[i].chromosome + 1<<" ";
    	fFile << loci[i].locusIdx+1 << " ";
   	}
	fFile << std::endl << std::endl;
	
	fFile << "# Frequencies used in creating this model: \n";
	
	for(int i=0; i<num_loci; i++){
		fFile << "#              "<<loci[i].alFreq1 << " " << loci[i].alFreq2 << std::endl;
  	}
