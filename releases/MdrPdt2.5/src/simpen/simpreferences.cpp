...
void SimPreferences::AddDiseaseLocus(int chr, int idx, float fr1, float fr2) {
	diseaseLoci.push_back(DiseaseLocus(chr, idx, fr1, fr2));
}
...
void SimPreferences::read_config(string configname = ""){
  ...
  while(!config.eof()){
	...
	if (key.compare(ADDLOCUS) == 0) {
		...
		AddDiseaseLocus(chr, idx, af1, af2);
	}
