...
This is similar to the sorted text report, but it doesn't bother with separating 
varied size models. 

	@author Eric Torstenson <torstenson@chgr.mc.vanderbilt.edu>
*/
class PowerRepos : public SnpRecipient, public SnpReporter
{
public:
struct ReportModel {
...
protected:
	ReportArray contents;
	uint snpCount;										///<The number of snps to be stored
};

inline
PowerRepos::PowerRepos(uint snpCount) : distribution(NULL), snpCount(snpCount) {
}

inline
PowerRepos::~PowerRepos() {

	
}

inline
uint PowerRepos::GetTotalEntryCount(uint loci) {
	return contents.size();
}

inline
uint PowerRepos::GetEntryCount(uint loci) {
...
/**
 * @todo PowerRepos::GetReportEntry() should return an entry and the report should contruct entries...
 */
inline
string PowerRepos::GetReportEntry(uint loci, uint snpIdx) {
	if (snpIdx<contents.size() == 0)  
		return "";
	
	char basicEntry[128];
	ReportModel &model = contents[snpIdx];
	double pVal=0.0;
	if (distribution)	{
		...
		sprintf(basicEntry, "%s\t%.4f", model.GetLabel().c_str(), model.score);
	return basicEntry;
}

inline
void PowerRepos::Sort() {
	sort(contents.begin(), contents.end(),ReportEntryLT());
}
